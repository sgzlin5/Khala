...
typedef uint8_t  n2n_pairs_id_t[N2N_MAC_SIZE * 2];
...
typedef enum n2n_pc {
    ...
    n2n_punch_hello       =  13,
    n2n_punch_method      =  14,
    n2n_change_method     =  15,
    n2n_punch_done        =  16,
    n2n_n4_hello          =  17,
    n2n_n4_piinfo         =  18,
    n2n_juice_sdp         =  19,
    n2n_juice_candidate   =  20,
    n2n_p2p_ping          =  21,
    n2n_p2p_pong          =  22
} n2n_pc_t;

typedef enum peer_punch_status {
    peer_not_punched    = 0,
    peer_wait_punch,
    peer_punching,
    peer_punch_win,
...
#define KHALA_DEFAULT_VERIFY_URL        "http://10.51.133.99:8100/base/n2n/vnp/auth"
#define KHALA_MAX_BUF                   (128)

#ifdef HAVE_UBUS
#define CONNNECT_PERIOD (3 * 1000)
#define UBUS_TIMEOUT (3 * 1000)
#endif

#define EDGE_INET_ADDRSTRLEN        (16)
#define EDGE_CMD_STOP               "05!BBSTOP"

#define EDGE_N4_CAPS                (1 << 0)
#define EDGE_JUICE_CAPS             (1 << 1)
#define EDGE_ALL_PUNCH_CAPS         (EDGE_N4_CAPS | EDGE_JUICE_CAPS)
#define METHOD_NUM                  (2)
#define EDGE_EX_PORT                (62000)
#define EDGE_FAIL_PUNCH_SLEEP_TIME  (120)
...
typedef struct n2n_REGISTER_SUPER {
    ...
    uint8_t            encrypt_key[KHALA_MAX_BUF];
    int                punch_caps;
...
/* Common */
typedef struct n2n_HELLO {
    n2n_mac_t      srcMac;  /* Our Mac*/
    n2n_mac_t      peerMac; /* Peer Mac*/
} n2n_PUNCH_HELLO_t;

typedef struct n2n_METHOD {
    n2n_mac_t      srcMac;  /* Our Mac*/
    n2n_mac_t      peerMac; /* Peer Mac*/
    int            punch_method;
} n2n_PUNCH_METHOD_t;

typedef struct n2n_CHANGE {
    n2n_mac_t      srcMac;  /* Our Mac*/
    n2n_mac_t      peerMac; /* Peer Mac*/
} n2n_PUNCH_CHANGE_t;

typedef struct n2n_DONE {
    n2n_mac_t      srcMac;  /* Our Mac*/
    n2n_mac_t      peerMac; /* Peer Mac*/
} n2n_PUNCH_DONE_t;
/* Common End */
/* N4 */
typedef struct n4_HELLO {
    n2n_mac_t      srcMac;  /* Our Mac*/
    n2n_mac_t      peerMac; /* Peer Mac*/
    uint16_t       min_port;
    uint16_t       max_port;
...
typedef struct n4_PIINFO {
    n2n_mac_t      peerMac;     /* Peer Mac*/
    uint32_t       peerPort;    /* Peer Punch socket port */
    n2n_sock_t     sock;
    uint16_t       n4_type;
...
typedef struct juice_CANDIDATE {
    n2n_mac_t      srcMac;  /* Our Mac*/
    n2n_mac_t      peerMac; /* Peer Mac*/
    char candidate[N2N_MSG_BUF_SIZE];
} juice_CANDIDATE_t;
/* Juice End*/
/* P2P Detect */
typedef struct n2n_P2P_PING {
    n2n_mac_t      srcMac;  /* Our Mac*/
} n2n_P2P_PING_t;
typedef struct n2n_P2P_PONG {
    n2n_mac_t      peerMac;  /* Our Mac*/
} n2n_P2P_PONG_t;

typedef struct n2n_PUNCH_READY_ACK {
    n2n_cookie_t    cookie;    /**< Return cookie from REGISTER */
    n2n_mac_t       srcMac;    /**< MAC of acknowledging party (supernode or edge) */
    n2n_mac_t       dstMac;    /**< Reflected MAC of registering edge from REGISTER */
    n2n_sock_t      sock;      /**< Supernode's view of edge socket (IP Addr, port) */
} n2n_PUNCH_READY_ACK_t;
...
typedef struct n2n_punch_pair n2n_punch_pairs_t;
typedef struct khala_punch khala_punch_t;
typedef struct sn_community sn_community_t;
...
typedef struct n2n_edge_conf {
    ...
    edge_punch_status_value_t   punch_status;
    time_t                   last_notify;
...
typedef ssize_t (*sendto_sock_t)(n2n_sn_t *sss,
                           SOCKET socket_fd,
                           const struct sockaddr *socket,
                           const uint8_t *pktbuf,
                           size_t pktsize);
typedef int (*punching_t)(n2n_edge_t *eee);
typedef int (*punch_lose_t)(n2n_edge_t *eee);
typedef int (*punch_win_t)(n2n_edge_t *eee);
typedef void (*punch_stop_t)(n2n_edge_t *eee);
typedef void (*punch_cleanup_t)(n2n_edge_t *eee);
typedef void (*paris_fn_t)(n2n_sn_t *sss, struct sn_community *comm, n2n_punch_pairs_t *pairs, sendto_sock_t sendto_sock);
...
struct khala_punch {
    ...
    int punch_loop;
    punching_t punching;
    punch_lose_t punch_lose;
    punch_win_t punch_win;
    punch_stop_t punch_stop;
    punch_cleanup_t punch_cleanup;
    paris_fn_t paris_fn;
    void *ptr;
...
struct n2n_edge {
    ...
    struct peer_info *               curr_punch_peer;                    /**< Edges we are want to punch. */
    ...
    time_t                           last_punch;                         /**< Last time punch threads was performed. */
    ...
    pthread_t                        punch_thread_id;
    pthread_mutex_t                  punch_access;
    /* Socket used to punch */
    int                              ex_sock;
    int                              curr_punch_type;
    khala_punch_t                    *punch_list;
    khala_punch_t                    *curr_punch;
    int                              failed_times;
};
...
struct n2n_punch_pair {
    struct peer_info *edge1;
    struct peer_info *edge2;
    n2n_pairs_id_t pair_id;
    uint16_t cur_method;
    uint16_t failed_method;
    int rec_change;
...
struct sn_community {
    ...
    n2n_punch_pairs_t             *punch_pairs;
    time_t                        last_iter_pairs;
    UT_hash_handle hh;                                    /* makes this structure hashable */
};
