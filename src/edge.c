...
extern void send_register_super (n2n_edge_t *eee);
extern void send_query_peer (n2n_edge_t *eee, const n2n_mac_t dst_mac);
extern int supernode_connect (n2n_edge_t *eee);
extern void supernode_disconnect (n2n_edge_t *eee);
extern int fetch_and_eventually_process_data (n2n_edge_t *eee, SOCKET sock,
                                       uint8_t *pktbuf, uint16_t *expected, uint16_t *position,
                                       time_t now);
extern uint8_t resolve_check (n2n_resolve_parameter_t *param, uint8_t resolution_request, time_t now);
extern int edge_init_routes (n2n_edge_t *eee, n2n_route_t *routes, uint16_t num_routes);
...
static int setOption (int optkey, char *optargument, n2n_tuntap_priv_config_t *ec, n2n_edge_conf_t *conf) {
    ...
    switch(optkey) {
        ...
        case 'o': /* unprivileged uid */ {
            conf->notify_interval = atoi(optargument);
            break;
        }
...
static int loadFromCLI (int argc, char *argv[], n2n_edge_conf_t *conf, n2n_tuntap_priv_config_t *ec) {
    ...
    while ((c = getopt_long(argc, argv,
                            "k:a:c:Eu:g:m:M:o:s:d:l:p:fvhrt:i:I:J:P:S::DL:z::A::Hn:R:e:"
#ifdef __linux__
                            "T:"
#endif
#ifdef WIN32
...
static int ubus_init(void)
{
    ...
    if (!ubus_ctx) {
        traceEvent(TRACE_ERROR, "ubus connected failed");
        return -1;
    }
    traceEvent(TRACE_DEBUG, "ubus connected id is %08x", ubus_ctx->local_id);
    return 0;
}

static void ubus_deinit(void)
{
    if (ubus_ctx) {
        ubus_free(ubus_ctx);
    }
...
#ifdef HAVE_UBUS
    eee->conf.notify_interval = (eee->conf.notify_interval == 0) ? 30 : eee->conf.notify_interval;
    ubus_init();
#endif
