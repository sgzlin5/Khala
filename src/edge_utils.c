...
static void nat_type_test(n2n_edge_t *eee)
{
    uint16_t map_port[2] = {0};
    uint16_t local_port = STUN_TEST_PORT;
    int ret;
    int cnt = 0, type;
    ...
    while (1) {
first_stun:
        ret = send_stun_binding(eee, local_port, sockbuf, STUN_SERVER_PORT1, &map_port[0]);
        usleep(10000);
        if (ret < 0 && cnt < 5) {
            cnt++;
            goto first_stun;
...
second_stun:
        ret = send_stun_binding(eee, local_port, sockbuf, STUN_SERVER_PORT2, &map_port[1]);
        usleep(10000);
        if (ret < 0 && cnt < 5) {
            cnt++;
            goto second_stun;
    ...
    if (map_port[0] == map_port[1] && map_port[0] != 0 && map_port[1] != 0) {
        eee->conf.nat_type = STUN_EASY_NAT;
    } else {
        eee->conf.nat_type = STUN_HARD_NAT;
    }
    traceEvent(TRACE_NORMAL, "Local-Deivce %s-NAT Detected (Local Port [%d] Mapped Port[%d:%d])", 
        (eee->conf.nat_type == STUN_EASY_NAT) ? "Easy" : "Hard", local_port, map_port[0], map_port[1]);
}
...
static void register_with_new_peer (n2n_edge_t *eee,
    ...
    if(scan == NULL) {
        ...
        traceEvent(TRACE_NORMAL, "new pending peer %s [%s] ",
...
static void check_known_peer_sock_change (n2n_edge_t *eee,
    ...
    if(!sock_equal(&(scan->sock), peer)) {
        if(!from_supernode) {
            ...
            closesocket(scan->p2p_sock);
            scan->punch_status = peer_not_punched;
            HASH_DEL(eee->known_peers, scan);
            free(scan);
...
void send_query_peer (n2n_edge_t * eee,
    ...
    if(!is_null_mac(dst_mac)) {
        ...
        traceEvent(TRACE_NORMAL, "send QUERY_PEER to supernode");
...
void send_register_super (n2n_edge_t *eee) {
    ...
    memcpy(reg.encrypt_key, eee->conf.encrypt_key, KHALA_MAX_BUF);
...
static unsigned int readMgmtJson(char *buff, n2n_edge_t *eee)
{
    struct json_object *json = json_object_new_object();
    struct json_object *forward, *p2p, *server;
    const char *jstr;
    struct json_object *tmp;
    struct peer_info *peer, *tmpPeer;
    in_addr_t net;
    char ip_buf[N2N_SOCKBUF_SIZE];
    macstr_t mac_buf;
    time_t now;
    ...
    now = time(NULL);
    /* common */
    json_object_object_add(json, "community", json_object_new_string((eee->conf.header_encryption == HEADER_ENCRYPTION_NONE) ? (char*)eee->conf.community_name : ""));
    json_object_object_add(json, "uptime", json_object_new_int(now - eee->start_time));
    json_object_object_add(json, "tx", json_object_new_int((unsigned int) eee->stats.tx_p2p + eee->stats.tx_sup));
    json_object_object_add(json, "rx", json_object_new_int((unsigned int) eee->stats.rx_p2p + eee->stats.rx_sup));
    json_object_object_add(json, "p2p_tx", json_object_new_int((unsigned int) eee->stats.tx_p2p));
    json_object_object_add(json, "p2p_rx", json_object_new_int((unsigned int) eee->stats.rx_p2p));
    json_object_object_add(json, "fw_tx", json_object_new_int((unsigned int) eee->stats.tx_sup));
    json_object_object_add(json, "fw_rx", json_object_new_int((unsigned int) eee->stats.rx_sup));

    forward = json_object_new_array();
    p2p = json_object_new_array();
    server = json_object_new_array();
    json_object_object_add(json, "forward", forward);
    json_object_object_add(json, "p2p", p2p);
    json_object_object_add(json, "server", server);

    /* forward */
    HASH_ITER(hh, eee->pending_peers, peer, tmpPeer) {
        tmp = json_object_new_object();
        net = htonl(peer->dev_addr.net_addr);
        json_object_object_add(tmp, "ip", json_object_new_string((peer->dev_addr.net_addr == 0) ? "" : inet_ntoa(*(struct in_addr *) &net)));
        json_object_object_add(tmp, "mac", json_object_new_string((is_null_mac(peer->mac_addr)) ? "" : macaddr_str(mac_buf, peer->mac_addr)));
        json_object_object_add(tmp, "outip", json_object_new_string(sock_to_ip(ip_buf, &(peer->sock))));
        json_object_object_add(tmp, "outport", json_object_new_int(peer->sock.port));
        json_object_object_add(tmp, "hostname", json_object_new_string((const char *)peer->dev_desc));
        json_object_object_add(tmp, "last", json_object_new_int((unsigned int)(now - peer->last_seen)));
        json_object_array_add(forward, tmp);
    }
    ...
    HASH_ITER(hh, eee->known_peers, peer, tmpPeer) {
        ...
        json_object_object_add(tmp, "hostname", json_object_new_string((const char *)peer->dev_desc));
        json_object_object_add(tmp, "last", json_object_new_int((unsigned int)(now - peer->last_seen)));
        json_object_array_add(p2p, tmp);
    }

    /* server */
    HASH_ITER(hh, eee->conf.supernodes, peer, tmpPeer) {
        tmp = json_object_new_object();
        json_object_object_add(tmp, "ip", json_object_new_string(sock_to_ip(ip_buf, &(peer->sock))));
        json_object_object_add(tmp, "port", json_object_new_int(peer->sock.port));
        json_object_object_add(tmp, "last", json_object_new_int((unsigned int)(now - peer->last_seen)));
        json_object_object_add(tmp, "uptime", json_object_new_int((unsigned int)(peer->uptime)));
        json_object_array_add(server, tmp);
    }

    memset(buff, 0, N2N_PKT_BUF_SIZE);
    msg_len = 0;
    jstr = json_object_to_json_string(json);
    msg_len += snprintf((char *) (buff + msg_len), (N2N_PKT_BUF_SIZE - msg_len), "%s\n", jstr);
    if (json) {
...
static void readFromMgmtSocket (n2n_edge_t *eee) {
    ...
    if(0 == memcmp(udp_buf, "info", 4)) {
        msg_len = 0;
        msg_len += readMgmtJson(udp_buf, eee);
        sendto(eee->udp_mgmt_sock, udp_buf, msg_len, 0/*flags*/,
                (struct sockaddr *) &sender_sock, sizeof(struct sockaddr_in));
        return;
    }
...
static int find_peer_destination (n2n_edge_t * eee,
    ...
    if(scan && (scan->last_seen > 0)) {
        if((now - scan->last_p2p) >= (scan->timeout / 2)) {
            ...
            if (scan->p2p_sock != -1) {
                destination->port = ntohs(scan->punch_sock.sin_port);
            }
            retval = 1;
        }
    }
    ...
    if(retval == 0) {
        HASH_FIND_PEER(eee->pending_peers, mac_address, fw_peer);
        if (fw_peer != NULL && fw_peer->punch_status == peer_not_punched) {
            traceEvent(TRACE_DEBUG, "Add peer to punching list");
            fw_peer->punch_status = peer_wait_punch;
        }
        memcpy(destination, &(eee->curr_sn->sock), sizeof(struct sockaddr_in));
...
static int send_packet (n2n_edge_t * eee,
    ...
    struct peer_info *peer, *tmp_peer, *p2p_peer;
    int p2p_sock;
    ...
    if (is_p2p) {
        traceEvent(TRACE_INFO, "[p2p]: Tx PACKET of %u bytes to %s [%s]",
               pktlen, macaddr_str(mac_buf, dstMac),
               sock_to_cstr(sockbuf, &destination));
    } else {
        traceEvent(TRACE_INFO, "[pSp]: Tx PACKET of %u bytes to %s [%s]",
               pktlen, macaddr_str(mac_buf, dstMac),
               sock_to_cstr(sockbuf, &destination));
    }
    ...
    if (is_p2p) {
        /* p2p_peer never be NULL */
        HASH_FIND_PEER(eee->known_peers, dstMac, p2p_peer);
        fill_sockaddr((struct sockaddr *) &addr, sizeof(addr), &destination);
        p2p_sock = (p2p_peer->p2p_sock != -1) ? p2p_peer->p2p_sock : eee->sock;
        sendto(p2p_sock,  pktbuf, pktlen, 0, (struct sockaddr *)&addr, sizeof(struct sockaddr_in));
    } else {
        ...
        if(eee->conf.drop_multicast &&
           (is_ip6_discovery(eth_pkt, len) ||
            is_ethMulticast(eth_pkt, len))) {
                traceEvent(TRACE_DEBUG, "dropping Tx multicast");
        } else {
            if(!eee->last_sup) {
...
/* ************************************** */
static void edge_remove_failed_peer(n2n_edge_t *eee)
{
    struct peer_info *fail_peer, *tmp;
    int gap;

    gap = (eee->failed_times > 0) ? eee->failed_times : 1;
    if (time(NULL) - eee->last_punch > (EDGE_FAIL_PUNCH_SLEEP_TIME * gap)) {
        HASH_ITER(hh, eee->pending_peers, fail_peer, tmp) {
            if (fail_peer && fail_peer->punch_status == peer_punch_lose) {
                fail_peer->punch_status = peer_not_punched;
            }
        }
    }
}

static void edge_start_punch(n2n_edge_t *eee)
{
    struct peer_info *scan, *tmp;

    if (eee->conf.punch_status >= edge_punch_punching) {
        return;
    }

    HASH_ITER(hh, eee->pending_peers, scan, tmp) {
        if (scan && scan->punch_status == peer_wait_punch) {
            eee->curr_punch_peer = scan;
            eee->conf.punch_status = edge_punch_punching;
            scan->punch_status = peer_punching;
            traceEvent(TRACE_NORMAL, " => Starting punch \n");
            punch_create_thread(eee);
            break;
        }
    }
}

static void send_p2p_ping(n2n_edge_t *eee, struct peer_info *peer, n2n_pc_t ping_pong)
{
    n2n_common_t common;
    n2n_P2P_PING_t ping;
    size_t idx;
    ...
    idx = 0;
    encode_mac(ping.srcMac, &idx, eee->device.mac_addr);
    idx = 0;
    encode_P2P_PING(pktbuf, &idx, &common, &ping);
    sendto(peer->p2p_sock,  pktbuf, idx, 0, (struct sockaddr *)&peer->punch_sock, sizeof(struct sockaddr_in));
    eee->stats.tx_p2p++;
}

static void edge_update_p2p(n2n_edge_t *eee)
{
    struct peer_info *scan, *tmp;
    time_t now;

    now = time(NULL);
    HASH_ITER(hh, eee->known_peers, scan, tmp) {
        if (scan && scan->punch_status == peer_punch_win) {
            if ((now - scan->last_p2p) > (scan->timeout / 10)) {
                send_p2p_ping(eee, scan, n2n_p2p_ping);
            }
        }
    }
}

static void edge_update_status(n2n_edge_t *eee)
{
    traceEvent(TRACE_DEBUG, "==> Punch Status %d", eee->conf.punch_status);
    switch(eee->conf.punch_status) {
        case edge_punch_ready: {
            edge_start_punch(eee);
            break;
        }
        case edge_punch_win: {
            eee->conf.punch_status = edge_punch_ready;
            break;
        }
        case edge_punch_lose: {
            eee->conf.punch_status = edge_punch_ready;
            break;
        }
        case edge_punch_init:   /* fall through */
        case edge_punch_punching: {
            break;
        }
    }
    edge_remove_failed_peer(eee);
    /* Send P2P Ping to all p2p peers */
    edge_update_p2p(eee);
}
    ...
    if(0 == memcmp(cmn.community, eee->conf.community_name, N2N_COMMUNITY_SIZE)) {
        switch(msg_type) {
            ...
            case MSG_TYPE_REGISTER_SUPER_ACK: {
                ...
                eee->conf.punch_status = (eee->conf.punch_status == edge_punch_init) ? edge_punch_ready : eee->conf.punch_status;
            ...
            case MSG_TYPE_PEER_INFO: {
                ...
                if(is_null_mac(pi.mac)) {
                    ...
                    if(scan) {
                        if (scan->p2p_sock > 0) {
                            pi.sock.port = ntohs(scan->punch_sock.sin_port);
                        }
                        scan->sock = pi.sock;

                        traceEvent(TRACE_NORMAL, "Rx PEER_INFO %s can be found at [%s] %d",
                                   macaddr_str(mac_buf1, pi.mac),
                                   sock_to_cstr(sockbuf1, &pi.sock),
                                   ntohs(scan->punch_sock.sin_port));
            ...
            case MSG_TYPE_UNREGISTER_SUPER: {
                ...
                HASH_FIND_PEER(eee->pending_peers, unreg.srcMac, peer);
                if(peer != NULL) {
                    if (eee->conf.punch_status >= edge_punch_punching &&
                        eee->curr_punch_peer != NULL &&
                        !memcmp(eee->curr_punch_peer->mac_addr, peer->mac_addr, sizeof(n2n_mac_t))) {
                        eee->conf.punch_status = edge_peer_lost;
                    } else {
                        pthread_mutex_lock(&eee->punch_access);
                    ...
                    HASH_FIND_PEER(eee->known_peers, unreg.srcMac, peer);
                    if (peer != NULL) {
                        HASH_DEL(eee->known_peers, peer);
                        free(peer);
                    }
            ...
            case MSG_TYPE_P2P_PING:
            case MSG_TYPE_P2P_PONG: {
                eee->stats.rx_p2p++;
                /* Receive Ping from p2p peer */
                n2n_P2P_PING_t ping;
                struct peer_info *scan;

                decode_P2P_PING(&ping, &cmn, udp_buf, &rem, &idx);
                HASH_FIND_PEER(eee->known_peers, ping.srcMac, scan);
                if (scan != NULL) {
                    now = time(NULL);
                    scan->last_p2p = now;
                    scan->last_seen = now;
                    if (msg_type == MSG_TYPE_P2P_PING) {
                        send_p2p_ping(eee, scan, n2n_p2p_pong);
                    }
                }
                break;
            }
        ...
        traceEvent(TRACE_INFO, "received packet with unknown community %s", cmn.community);
    else
...
/* ************************************** */
#ifdef HAVE_UBUS
void ubus_notify_mqlink(n2n_edge_t *eee)
{
    int ret;
    uint32_t mqlink_id;
    struct json_object *msg;
    struct json_object *forward, *p2p, *server, *tmp;
    struct peer_info *peer, *tmpPeer;
    macstr_t mac_buf;
    char time_buf[10];
    char uptime_buf[11];
    ...
    mqlink_id = 0;
    ret = ubus_lookup_id(ubus_ctx, "shinya_mqlink", &mqlink_id);
    traceEvent(TRACE_DEBUG, "lookup id %d ret %d", mqlink_id, ret);
    if (ret != UBUS_STATUS_OK) {
        traceEvent(TRACE_DEBUG, "ubus look shinya_mqlink failed %d", ret);
        return;
    }
    memset(&blob_msg, 0, sizeof(struct blob_buf));
    blob_buf_init(&blob_msg, 0);
    msg = json_object_new_object();
    forward = json_object_new_array();
    p2p = json_object_new_array();
    server = json_object_new_array();
    /* add msg here */
    /* ubus call mqlink status_notify '{"user":"apmgr","name":"topology","bizid":"sta_apmgr_0","ack":"false","val":{"status":"topology"}}' */
    blobmsg_add_string(&blob_msg, "user", "khala");
    blobmsg_add_string(&blob_msg, "name", "khala");
    blobmsg_add_string(&blob_msg, "bizid", "sta_khala_0");
    blobmsg_add_string(&blob_msg, "ack", "false");

    json_object_object_add(msg, "community", json_object_new_string((const char *)eee->conf.community_name));
    json_object_object_add(msg, "uptime", json_object_new_int64(time(NULL) - eee->start_time));
    json_object_object_add(msg, "fw_tx", json_object_new_int64(eee->stats.tx_sup));
    json_object_object_add(msg, "fw_rx", json_object_new_int64(eee->stats.rx_sup));
    json_object_object_add(msg, "p2p_tx", json_object_new_int64(eee->stats.tx_p2p));
    json_object_object_add(msg, "p2p_rx", json_object_new_int64(eee->stats.rx_p2p));
    json_object_object_add(msg, "tx", json_object_new_int64(eee->stats.tx_sup + eee->stats.tx_p2p));
    json_object_object_add(msg, "rx", json_object_new_int64(eee->stats.rx_sup + eee->stats.rx_p2p));
    traceEvent(TRACE_DEBUG, "common msg %s", json_object_to_json_string(msg));
    /* Forward */
    HASH_ITER(hh, eee->pending_peers, peer, tmpPeer) {
        net = htonl(peer->dev_addr.net_addr);
        snprintf(time_buf, sizeof(time_buf), "%9u", (unsigned int)(time(NULL) - peer->last_seen));
        tmp = json_object_new_object();
        json_object_object_add(tmp, "ip", json_object_new_string((peer->dev_addr.net_addr == 0) ? "" : inet_ntoa(*(struct in_addr *) &net)));
        json_object_object_add(tmp, "mac", json_object_new_string((is_null_mac(peer->mac_addr)) ? "" : macaddr_str(mac_buf, peer->mac_addr)));
        json_object_object_add(tmp, "outip", json_object_new_string(sock_to_ip(sockbuf, &(peer->sock))));
        json_object_object_add(tmp, "outport", json_object_new_int(peer->sock.port));
        json_object_object_add(tmp, "hostname", json_object_new_string((const char *)peer->dev_desc));
        json_object_object_add(tmp, "last", json_object_new_string((peer->last_seen) ? time_buf : ""));
        json_object_array_add(forward, tmp);
    }
    traceEvent(TRACE_DEBUG, "forward msg %s", json_object_to_json_string(forward));
    json_object_object_add(msg, "forward", forward);
    /* P2P */
    HASH_ITER(hh, eee->known_peers, peer, tmpPeer) {
        net = htonl(peer->dev_addr.net_addr);
        snprintf(time_buf, sizeof(time_buf), "%9u", (unsigned int)(time(NULL) - peer->last_seen));
    ...
    traceEvent(TRACE_DEBUG, "p2p msg %s", json_object_to_json_string(p2p));
    json_object_object_add(msg, "p2p", p2p);
    /* Supernode */
    HASH_ITER(hh, eee->conf.supernodes, peer, tmpPeer) {
        net = htonl(peer->dev_addr.net_addr);
        snprintf(time_buf, sizeof(time_buf), "%u", (unsigned int)(time(NULL) - peer->last_seen));
        snprintf(uptime_buf, sizeof(uptime_buf), "%u", (unsigned int)(peer->uptime));
        tmp = json_object_new_object();
        json_object_object_add(tmp, "ip", json_object_new_string(sock_to_ip(sockbuf, &(peer->sock))));
        json_object_object_add(tmp, "port", json_object_new_int(peer->sock.port));
        json_object_object_add(tmp, "last", json_object_new_string(((peer->last_seen) ? time_buf : "")));
        json_object_object_add(tmp, "uptime", json_object_new_string(((peer->uptime) ? uptime_buf : "")));
        json_object_array_add(server, tmp);
    }
    traceEvent(TRACE_INFO, "server msg %s", json_object_to_json_string(server));
    json_object_object_add(msg, "server", server);
    table = blobmsg_open_table(&blob_msg, "val");
    blobmsg_add_object(&blob_msg, msg);
    blobmsg_close_table(&blob_msg, table);
    str = blobmsg_format_json(blob_msg.head, true);
    if (str) {
        traceEvent(TRACE_DEBUG, "ubus json str %s ", str);
        free(str);
    }
    ret = ubus_invoke(ubus_ctx, mqlink_id, "status_notify", blob_msg.head, NULL, NULL, UBUS_TIMEOUT);
    if (ret != 0) {
        traceEvent(TRACE_DEBUG, "ubus_invoke mqlink id %d failed %d", mqlink_id, ret);
    }

    json_object_put(msg);
    blob_buf_free(&blob_msg);
}
#endif
...
int run_edge_loop (n2n_edge_t *eee) {
    ...
    *eee->keep_running = 1;
     ...
     */
    eee->conf.punch_caps = EDGE_ALL_PUNCH_CAPS;
    eee->last_punch = time(NULL);
    eee->conf.punch_status = edge_punch_init;
    eee->conf.last_notify = time(NULL);
    eee->ex_sock = -1;
    pthread_mutex_init(&eee->punch_access, NULL);
    eee->punch_list = NULL;
    edge_punch_init_list(eee);
    nat_type_test(eee);
    while(*eee->keep_running) {
        traceEvent(TRACE_DEBUG, "edge running");
        int rc, max_sock = 0;
        fd_set socket_mask;
        struct timeval wait_time;
        time_t now;
        struct peer_info *p2p_peer, *tmp_p2p;
...
#ifdef HAVE_UBUS
        if ((time(NULL) - eee->conf.last_notify) > eee->conf.notify_interval) {
            ubus_notify_mqlink(eee);
            eee->conf.last_notify = time(NULL);
        }
#endif
        if(eee->sock >= 0) {
...
#endif
        HASH_ITER(hh, eee->known_peers, p2p_peer, tmp_p2p) {
            if (p2p_peer->p2p_sock > -1) {
                FD_SET(p2p_peer->p2p_sock, &socket_mask);
                max_sock = max(max_sock, p2p_peer->p2p_sock);
            }
        }
        ...
        if(rc > 0) {
            HASH_ITER(hh, eee->known_peers, p2p_peer, tmp_p2p) {
                if (p2p_peer->p2p_sock > -1) {
                    if(FD_ISSET(p2p_peer->p2p_sock, &socket_mask)) {
                        if(0 != fetch_and_eventually_process_data(eee, p2p_peer->p2p_sock,
...
static void edge_stop_thread(n2n_edge_t * eee)
{
    struct sockaddr_in peer_addr;
    ...
    sendto(eee->ex_sock, EDGE_CMD_STOP, strlen(EDGE_CMD_STOP), 0, (struct sockaddr*)&peer_addr, sizeof(peer_addr));
    if (eee->curr_punch != NULL) {
        eee->curr_punch->punch_stop(eee);
    }
    return;
}
...
void edge_term (n2n_edge_t * eee) {
    ...
    if (eee->conf.punch_status == edge_punch_punching) {
        traceEvent(TRACE_NORMAL, "punching thread stopping");
        punch_stop = true;
        edge_stop_thread(eee);
        pthread_join(eee->punch_thread_id, NULL);
        traceEvent(TRACE_NORMAL, "punching thread stopped");
    }

    clear_peer_list(&eee->pending_peers);
    clear_peer_list(&eee->known_peers);
    edge_punch_clean_list(eee);
    ...
    traceEvent(TRACE_NORMAL, "edge term");
    closeTraceFile();
