...
extern char * sock_to_ip (n2n_sock_str_t out,
    ...
    if(AF_INET6 == sock->family) {
        ...
        snprintf(out, N2N_SOCKBUF_SIZE, "XXXX");
        return out;
    } else {
        const uint8_t * a = sock->addr.v4;

        snprintf(out, N2N_SOCKBUF_SIZE, "%hu.%hu.%hu.%hu",
...
void punch_type_to_string(int type, n2n_desc_t name)
{
    switch(type) {
        case EDGE_JUICE_CAPS:
            strncpy(name, "JUICE", sizeof(n2n_desc_t));
            break;
        case EDGE_N4_CAPS:
            strncpy(name, "N4", sizeof(n2n_desc_t));
            break;
    }
    return;
}
...
static void parse_stun_response(uint8_t *response, size_t response_length, uint16_t *map_port) {
    if (response_length < sizeof(stun_message_header_t)) {
        traceEvent(TRACE_WARNING, "Response too short\n");
        return;
    }
    ...
    if (header->type != STUN_BINDING_RESPONSE) {
        traceEvent(TRACE_WARNING, "Not a binding response\n");
        return;
    }
    ...
    while (remaining_length > 0) {
        ...
        if (attr_length + 4 > remaining_length) {
            traceEvent(TRACE_WARNING, "Invalid attribute length\n");
            return;
        }
        ...
        if (attr_type == STUN_ATTR_MAPPED_ADDRESS) {
            ...
            traceEvent(TRACE_DEBUG, "Mapped Port:%d\n", port);
            *map_port = port;
            return;
            ...
            traceEvent(TRACE_DEBUG, "XOR-Mapped Address: %d\n", port);
            *map_port = port;
            return;
        }
...
int send_stun_binding(n2n_edge_t *eee, uint16_t local_port, char *stun_server, uint16_t server_port, uint16_t *map_port)
{
    int sockfd;
    ...
    if ((sockfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        traceEvent(TRACE_WARNING, "socket creation failed\n");
        return -1;
    }
    ...
    generate_transaction_id(msg.transaction_id);
    ...
    if (bind(sockfd,(struct sockaddr*) &local_addr, sizeof(local_addr)) < 0) {
        traceEvent(TRACE_WARNING, "bind failed\n");
        close(sockfd);
        return -1;
    }
    FD_ZERO(&readfds);
    ...
    if (sendto(sockfd, &msg, sizeof(msg), 0, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
        traceEvent(TRACE_WARNING, "sendto failed\n");
        close(sockfd);
        return -1;
    }

    traceEvent(TRACE_INFO, "STUN request sent to %s:%d\n", stun_server, server_port);

    wait_time.tv_sec = 1;
    wait_time.tv_usec = 0;
    rc = select(sockfd + 1, &readfds, NULL, NULL, &wait_time);
    if (rc > 0) {
        traceEvent(TRACE_DEBUG, "STUN response received\n");
        response_length = recvfrom(sockfd, response, sizeof(response), 0, (struct sockaddr *)&server_addr, &addr_len);
        if (response_length < 0) {
            traceEvent(TRACE_WARNING, "recvfrom failed\n");
            close(sockfd);
            return -1;
        }
    } else {
        traceEvent(TRACE_WARNING, "Stun receive timeout\n");
        close(sockfd);
        return -1;
    }

    parse_stun_response(response, response_length, map_port);
    close(sockfd);
    return 0;
}