...
/* ************************************** */
static int punch_method_list[METHOD_NUM] = {
    EDGE_N4_CAPS,
    EDGE_JUICE_CAPS
};
...
int comm_init (struct sn_community *comm, char *cmn) {
    ...
    comm->punch_pairs = NULL;
    comm->last_iter_pairs = time(NULL);
...
void sn_term (n2n_sn_t *sss) {
    ...
    node_supernode_association_t *assoc, *tmp_assoc;
    n2n_punch_pairs_t *pairs, *tmp_pairs;
    ...
    HASH_ITER(hh, sss->communities, community, tmp) {
        ...
        /* remove all paris */
        HASH_ITER(hh, community->punch_pairs, pairs, tmp_pairs) {
            HASH_DEL(community->punch_pairs, pairs);
            free(pairs);
        }
    ...
    /* Not known */
        if(handle_remote_auth(sss, &(reg->auth), answer_auth, comm) == 0) {
            if(skip_add == SN_ADD) {
                ...
                scan->punch_caps = reg->punch_caps;
                HASH_ADD_PEER(comm->edges, scan);
...
size_t curl_callback(void *contents, size_t size, size_t nmemb, char *userdata) {
    size_t realsize;

    realsize = size * nmemb;
    realsize = (realsize > N2N_PKT_BUF_SIZE) ? N2N_PKT_BUF_SIZE : realsize;
    memcpy(userdata, contents, realsize);
    userdata[realsize] = 0;
    return realsize;
}

static int supernode_verify_community(char * url, char *cmn_name, char* paswd)
{
    char curl_http[N2N_PKT_BUF_SIZE];
    CURL *curl;
    CURLcode res;
    struct curl_slist *headers = NULL;
    int ret = -1;
    char userdata[N2N_PKT_BUF_SIZE];

    curl_global_init(CURL_GLOBAL_DEFAULT);
    curl = curl_easy_init();

    if (curl) {
        headers = curl_slist_append(headers, "feign: true");
        snprintf(curl_http, sizeof(curl_http) - 1, "%s?community=%s&secret=%s", url, cmn_name, paswd);
        curl_easy_setopt(curl, CURLOPT_URL, curl_http);
        curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);
        curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, curl_callback);
        curl_easy_setopt(curl, CURLOPT_WRITEDATA, userdata);
        res = curl_easy_perform(curl);
        if(res != CURLE_OK) {
            traceEvent(TRACE_ERROR, "curl http %s failed %s", curl_http, curl_easy_strerror(res));
        } else {
            traceEvent(TRACE_DEBUG, "Received data: %s\n", userdata);
            struct json_object *parsed_json;
            struct json_object *data;
            struct json_object *code;

            parsed_json = json_tokener_parse(userdata);
            json_object_object_get_ex(parsed_json, "data", &data);
            json_object_object_get_ex(parsed_json, "code", &code);
            if (json_object_get_int(code) == 0 && json_object_get_boolean(data)) {
                ret = 0;
            }
        }
    }
...
static void update_punch_pairs(struct sn_community *comm, struct peer_info *peer)
{
    n2n_punch_pairs_t   *pairs, *tmp_pairs;

    HASH_ITER(hh, comm->punch_pairs, pairs, tmp_pairs) {
        if (!memcmp(pairs->edge1->mac_addr, peer->mac_addr, sizeof(n2n_mac_t)) ||
            !memcmp(pairs->edge2->mac_addr, peer->mac_addr, sizeof(n2n_mac_t))) {
            HASH_DEL(comm->punch_pairs, pairs);
            free(pairs);
        }
    }
}

static void run_punch_loop(n2n_sn_t *sss)
{
    struct sn_community *comm, *tmp_comm;
    n2n_punch_pairs_t   *pairs, *tmp_pairs;
    khala_punch_t *punch;
    ...
    now = time(NULL);
    HASH_ITER(hh, sss->communities, comm, tmp_comm) {
        comm->last_iter_pairs = now;
        HASH_ITER(hh, comm->punch_pairs, pairs, tmp_pairs) {
            memset(name, 0, sizeof(n2n_desc_t));
            punch_type_to_string(punch_method_list[pairs->cur_method % METHOD_NUM] , name);
            HASH_FIND_PUNCH(sss->punch_list, name, punch);
            traceEvent(TRACE_DEBUG, "Have pairs Status: [%d:%d] Port: [%d:%d]\n", pairs->edge1->ok, pairs->edge2->ok, pairs->edge1->punch_port, pairs->edge2->punch_port);
            traceEvent(TRACE_DEBUG, "Method Name: %s", name);
            if (punch != NULL && punch->paris_fn != NULL && pairs->edge1->ok == 1 && pairs->edge2->ok == 1) {
                punch->paris_fn(sss, comm, pairs, sendto_sock);
            }
        }
    }
}
...
static int process_udp (n2n_sn_t * sss,
    ...
    switch(msg_type) {
        ...
        case MSG_TYPE_REGISTER_SUPER: {
            ...
            if (sss->verify_url && supernode_verify_community(sss->verify_url, (char *)cmn.community, reg.encrypt_key) != 0) {
                traceEvent(TRACE_ERROR, "%s:%s Verify Failed\n", cmn.community, reg.encrypt_key);
                if (comm && comm->purgeable == COMMUNITY_PURGEABLE) {
                    node_supernode_association_t *assoc, *tmp_assoc;
                    traceEvent(TRACE_INFO, "purging register failed community %s", comm->community);
                    if(NULL != comm->header_encryption_ctx_static) {
                        /* this should not happen as 'purgeable' and thus only communities w/o encrypted header here */
                        free(comm->header_encryption_ctx_static);
                        free(comm->header_iv_ctx_static);
            ...
            traceEvent(TRACE_DEBUG, "%s:%s Verify Passed\n", cmn.community, reg.encrypt_key);
            ...
            if(peer != NULL) {
                if((auth = auth_edge(&(peer->auth), &unreg.auth, NULL, comm)) == 0) {
                    if((peer->socket_fd != sss->sock) && (peer->socket_fd >= 0)) {
                        ...
                        update_punch_pairs(comm, peer);
                        HASH_DEL(comm->edges, peer);
                        free(peer);
                    }
                    HASH_ITER(hh, comm->edges, peer, tmp_peer) {
                        /* Avoid lose */
                        sendto_peer(sss, peer, udp_buf, udp_size);
                        sendto_peer(sss, peer, udp_buf, udp_size);
                        sendto_peer(sss, peer, udp_buf, udp_size);
                    }
                }
        ...
        case MSG_TYPE_QUERY_PEER: {
            ...
            uint8_t                                encbuf_sender[N2N_SN_PKTBUF_SIZE];
            size_t                                 encx = 0;
            n2n_common_t                           cmn2;
            n2n_PEER_INFO_t                        pi_sender;
            struct sn_community_regular_expression *re, *tmp_re;
            int8_t                                 allowed_match = -1;
            uint8_t                                match = 0;
            int                                    match_length = 0;
            ...
            if(is_null_mac(query.targetMac)) {
                ...
                pi_sender.aflags = 0;
                memcpy(pi_sender.mac, query.targetMac, sizeof(n2n_mac_t));
                memcpy(pi_sender.srcMac, sss->mac_addr, sizeof(n2n_mac_t));
                pi_sender.sock.family = AF_INET;
                pi_sender.sock.port = ntohs(sender_sock->sin_port);
                memcpy(pi_sender.sock.addr.v4, &(sender_sock->sin_addr.s_addr), IPV4_SIZE);
                pi_sender.load = sn_selection_criterion_gather_data(sss);

                snprintf(pi_sender.version, sizeof(pi_sender.version), "%s", sss->version);
                pi_sender.uptime = now - sss->start_time;

                encode_PEER_INFO(encbuf_sender, &encx, &cmn2, &pi_sender);

                if(comm) {
                    if(comm->header_encryption == HEADER_ENCRYPTION_ENABLED) {
                        packet_header_encrypt(encbuf_sender, encx, encx, comm->header_encryption_ctx_dynamic,
                                              comm->header_iv_ctx_dynamic,
                                              time_stamp());
                    }
                }

                sendto_sock(sss, socket_fd, (struct sockaddr *)sender_sock, encbuf_sender, encx);
                ...
                if(scan) {
                    ...
                    pi_sender.aflags = 0;
                    memcpy(pi_sender.srcMac, query.srcMac, sizeof(n2n_mac_t));
                    memcpy(pi_sender.mac, query.targetMac, sizeof(n2n_mac_t));
                    pi_sender.sock = scan->sock;
                    if(scan->preferred_sock.family != (uint8_t)AF_INVALID) {
                        cmn2.flags |= N2N_FLAGS_SOCKET;
                        pi_sender.preferred_sock = scan->preferred_sock;
                    }
                    encode_PEER_INFO(encbuf_sender, &encx, &cmn2, &pi_sender);
                    if(comm->header_encryption == HEADER_ENCRYPTION_ENABLED) {
                        packet_header_encrypt(encbuf_sender, encx, encx, comm->header_encryption_ctx_dynamic,
                    ...
                    if(from_supernode) {
                        ...
                        encode_QUERY_PEER(encbuf_sender, &encx, &cmn2, &query);
                        ...
                        try_broadcast(sss, NULL, &cmn, query.srcMac, from_supernode, encbuf_sender, encx);
                    }
                }
            }
            break;
        ...
        case MSG_TYPE_PUNCH_HELLO: {
            n2n_common_t cmn2;
            n2n_PUNCH_HELLO_t hello;
            uint8_t encbuf[N2N_SN_PKTBUF_SIZE];
            size_t encx = 0;
            n2n_PUNCH_METHOD_t method;
            struct peer_info *punch_peer, *punch_sender;
            char  sender_buf[32];

            if(!comm) {
                traceEvent(TRACE_DEBUG, "dropped PUNCH_READY with unknown community %s", cmn.community);
                return -1;
            }

            if((from_supernode == 1) || (comm->is_federation == IS_FEDERATION)) {
                traceEvent(TRACE_DEBUG, "dropped PUNCH_READY: should not come from a supernode or federation.");
                return -1;
            }
            decode_PUNCH_HELLO(&hello, &cmn, udp_buf, &rem, &idx);
            HASH_FIND_PEER(comm->edges, hello.srcMac, punch_sender);
            HASH_FIND_PEER(comm->edges, hello.peerMac, punch_peer);
            traceEvent(TRACE_NORMAL, " <= Receive Hello from [sender: %s:%u] %s", intoa(ntohl(sender_sock->sin_addr.s_addr), sender_buf, sizeof(sender_buf)),
                        ntohs(sender_sock->sin_port), cmn.community);
            if (punch_peer != NULL && punch_sender != NULL) {
                if((comm->is_federation == IS_NO_FEDERATION) && (!is_null_mac(hello.srcMac) && !is_null_mac(hello.peerMac))) {
                    n2n_pairs_id_t pairs1_id, pairs2_id;
                    n2n_punch_pairs_t   *pairs1, *pairs2, *pairs = NULL;

                    memcpy(&pairs1_id, hello.srcMac, N2N_MAC_SIZE);
                    memcpy(&pairs1_id[N2N_MAC_SIZE], hello.peerMac, N2N_MAC_SIZE);
                    memcpy(&pairs2_id, hello.peerMac, N2N_MAC_SIZE);
                    memcpy(&pairs2_id[N2N_MAC_SIZE], hello.srcMac, N2N_MAC_SIZE);
                    
                    HASH_FIND_PAIRS(comm->punch_pairs, pairs1_id, pairs1);
                    HASH_FIND_PAIRS(comm->punch_pairs, pairs2_id, pairs2);
                    if ((pairs1 == NULL && pairs2 == NULL)) {
                        /* Alloc new pairs */
                        pairs = (n2n_punch_pairs_t*)calloc(1, sizeof(n2n_punch_pairs_t));
                        memcpy(pairs->pair_id, pairs1_id, N2N_MAC_SIZE * 2);
                        pairs->edge1 = punch_sender;
                        pairs->edge2 = punch_peer;
                        pairs->failed_method = 0;
                        pairs->cur_method = 0;
                        pairs->rec_change = METHOD_NUM;
                        HASH_ADD_PAIRS(comm->punch_pairs, pairs);
                    } else if (pairs1 || pairs2) {
                        pairs = (pairs1 != NULL) ? pairs1 : pairs2;
                        pairs->edge1 = punch_sender;
                        pairs->edge2 = punch_peer;
                    }
                    method.punch_method = punch_method_list[(pairs->cur_method % METHOD_NUM)];
                    cmn2.ttl = N2N_DEFAULT_TTL;
                    cmn2.pc = n2n_punch_method;
                    cmn2.flags = N2N_FLAGS_FROM_SUPERNODE;
                    memcpy(cmn2.community, cmn.community, N2N_COMMUNITY_SIZE);
                    encx = 0;
                    encode_mac(method.srcMac, &encx, hello.srcMac);
                    encx = 0;
                    encode_mac(method.peerMac, &encx, hello.peerMac);
                    encx = 0;
                    encode_PUNCH_METHOD(encbuf, &encx, &cmn2, &method);
                    traceEvent(TRACE_NORMAL, " => Send Method to sender");
        ...
        case MSG_TYPE_N4_HELLO: {
            ...
            decode_N4_HELLO(&hello, &cmn, udp_buf, &rem, &idx);
            HASH_FIND_PEER(comm->edges, hello.srcMac, punch_sender);
            HASH_FIND_PEER(comm->edges, hello.peerMac, punch_peer);
            traceEvent(TRACE_NORMAL, " <= Receive N4 Hello from [sender: %s:%u] %s", intoa(ntohl(sender_sock->sin_addr.s_addr), sender_buf, sizeof(sender_buf)),
                        ntohs(sender_sock->sin_port), cmn.community);

            if (punch_peer != NULL && punch_sender != NULL) {
                if((comm->is_federation == IS_NO_FEDERATION)) {
                    memset(&punch_sender->punch_sock, 0, sizeof(punch_sender->punch_sock));
                    memcpy(&punch_sender->punch_sock, sender_sock, sizeof(struct sockaddr_in));
                    punch_sender->punch_port = ntohs(sender_sock->sin_port);
                    cmn2.ttl = N2N_DEFAULT_TTL;
                    cmn2.pc = n2n_n4_hello;
                    cmn2.flags = N2N_FLAGS_FROM_SUPERNODE;
                    memcpy(cmn2.community, cmn.community, N2N_COMMUNITY_SIZE);
                    memset(&hello2, 0, sizeof(sizeof(n4_HELLO_t)));
                    encx = 0;
                    encode_mac(hello2.srcMac, &encx, hello.srcMac);
                    encx = 0;
                    encode_mac(hello2.peerMac, &encx, hello.peerMac);
                    encx = 0;
                    encode_N4_HELLO(encbuf, &encx, &cmn2, &hello2);
                    traceEvent(TRACE_NORMAL, " => Send N4 Hello to sender");
                    sendto_sock(sss, socket_fd, (struct sockaddr *)sender_sock, encbuf, encx);
                    punch_sender->ok = 1;
                    punch_sender->min_port = hello.min_port;
                    punch_sender->max_port = hello.max_port;
                    punch_sender->nat_type = hello.nat_type;
                }
            }
            break;
        }
        ...
        case MSG_TYPE_JUICE_CANDIDATE: {
            n2n_common_t cmn2;
            juice_CANDIDATE_t candidate, candidate2;
            uint8_t encbuf[N2N_SN_PKTBUF_SIZE];
            size_t encx = 0;
            struct peer_info *punch_peer, *punch_sender;

            if(!comm) {
                traceEvent(TRACE_DEBUG, "dropped PUNCH CHANGE with unknown community %s", cmn.community);
                return -1;
            }

            if((from_supernode == 1) || (comm->is_federation == IS_FEDERATION)) {
                traceEvent(TRACE_DEBUG, "dropped PUNCH CHANGE: should not come from a supernode or federation.");
                return -1;
            }
            memset(&candidate, 0, sizeof(juice_CANDIDATE_t));
            decode_CANDIDATE(&candidate, &cmn, udp_buf, &rem, &idx);
            HASH_FIND_PEER(comm->edges, candidate.srcMac, punch_sender);
            HASH_FIND_PEER(comm->edges, candidate.peerMac, punch_peer);
            if (punch_peer != NULL && punch_sender != NULL) {
                if((comm->is_federation == IS_NO_FEDERATION)) { 
                    if (punch_peer->ok == 2) {
                        cmn2.ttl = N2N_DEFAULT_TTL;
                        cmn2.pc = n2n_juice_candidate;
                        cmn2.flags = N2N_FLAGS_FROM_SUPERNODE;
                        memcpy(cmn2.community, cmn.community, N2N_COMMUNITY_SIZE);
                        memset(&candidate2, 0, sizeof(sizeof(juice_CANDIDATE_t)));
                        encx = 0;
                        encode_mac(candidate2.srcMac, &encx, candidate.srcMac);
                        encx = 0;
                        encode_mac(candidate2.peerMac, &encx, candidate.peerMac);
                        memcpy(candidate2.candidate, candidate.candidate, N2N_MSG_BUF_SIZE);
                        encx = 0;
                        encode_CANDIDATE(encbuf, &encx, &cmn2, &candidate2);
                        sendto_sock(sss, socket_fd, (struct sockaddr *)&punch_peer->punch_sock, encbuf, encx);
                        sendto_sock(sss, socket_fd, (struct sockaddr *)&punch_peer->punch_sock, encbuf, encx);
                        traceEvent(TRACE_NORMAL, "Rx JUICE Candidate Send to Peer");
                    }
                }
            }
            break;
        ...
        case MSG_TYPE_JUICE_SDP: {
            ...
            decode_SDP(&sdp, &cmn, udp_buf, &rem, &idx);
            HASH_FIND_PEER(comm->edges, sdp.srcMac, punch_sender);
            HASH_FIND_PEER(comm->edges, sdp.peerMac, punch_peer);
            if (punch_peer != NULL && punch_sender != NULL) {
                if((comm->is_federation == IS_NO_FEDERATION) && punch_sender->ok == 0) { 
                    memset(&punch_sender->punch_sock, 0, sizeof(punch_sender->punch_sock));
                    memcpy(&punch_sender->punch_sock, sender_sock, sizeof(struct sockaddr_in));
                    punch_sender->punch_port = ntohs(sender_sock->sin_port);
                    punch_sender->ok = 1;
                    memcpy(punch_sender->sdp, sdp.sdp, N2N_MSG_BUF_SIZE);
                }
            }
            break;
        }

        case MSG_TYPE_CHANGE_METHOD: {
            ...
            decode_PUNCH_CHANGE(&change, &cmn, udp_buf, &rem, &idx);
            HASH_FIND_PEER(comm->edges, change.srcMac, punch_sender);
            HASH_FIND_PEER(comm->edges, change.peerMac, punch_peer);
            traceEvent(TRACE_NORMAL, " <= Receive Change Method [sender: %s:%u] %s", intoa(ntohl(sender_sock->sin_addr.s_addr), sender_buf, sizeof(sender_buf)),
                        ntohs(sender_sock->sin_port), cmn.community);
            if (punch_peer != NULL && punch_sender != NULL) {
                if((comm->is_federation == IS_NO_FEDERATION)) {
                    n2n_pairs_id_t pairs1_id, pairs2_id;
                    ...
                    memcpy(&pairs1_id, change.srcMac, N2N_MAC_SIZE);
                    memcpy(&pairs1_id[N2N_MAC_SIZE], change.peerMac, N2N_MAC_SIZE);
                    memcpy(&pairs2_id, change.peerMac, N2N_MAC_SIZE);
                    memcpy(&pairs2_id[N2N_MAC_SIZE], change.srcMac, N2N_MAC_SIZE);
                    ...
                    if (pairs1 || pairs2) {
                        pairs = (pairs1 != NULL) ? pairs1 : pairs2;
                        pairs->rec_change++;
                        if ((pairs->rec_change % METHOD_NUM) == 0) {
                            pairs->cur_method++;
                        }
                    }
                }
        ...
        case MSG_TYPE_PUNCH_DONE: {
            n2n_PUNCH_DONE_t done;
            struct peer_info *punch_peer, *punch_sender;
            char  sender_buf[32];

            if(!comm) {
                traceEvent(TRACE_DEBUG, "dropped PUNCH DONE with unknown community %s", cmn.community);
                return -1;
            }

            if((from_supernode == 1) || (comm->is_federation == IS_FEDERATION)) {
                traceEvent(TRACE_DEBUG, "dropped PUNCH DONE: should not come from a supernode or federation.");
                return -1;
            }
            decode_PUNCH_DONE(&done, &cmn, udp_buf, &rem, &idx);
            HASH_FIND_PEER(comm->edges, done.srcMac, punch_sender);
            HASH_FIND_PEER(comm->edges, done.peerMac, punch_peer);
            traceEvent(TRACE_NORMAL, " <= Receive Punch Done [sender: %s:%u] %s", intoa(ntohl(sender_sock->sin_addr.s_addr), sender_buf, sizeof(sender_buf)),
                        ntohs(sender_sock->sin_port), cmn.community);
            if (punch_peer != NULL && punch_sender != NULL) {
                punch_sender->ok = 0;
                punch_peer->ok = 0;
                if((comm->is_federation == IS_NO_FEDERATION)) {
                    n2n_pairs_id_t pairs1_id, pairs2_id;
                    n2n_punch_pairs_t   *pairs1, *pairs2;

                    memcpy(&pairs1_id, done.srcMac, N2N_MAC_SIZE);
                    memcpy(&pairs1_id[N2N_MAC_SIZE], done.peerMac, N2N_MAC_SIZE);
                    memcpy(&pairs2_id, done.peerMac, N2N_MAC_SIZE);
                    memcpy(&pairs2_id[N2N_MAC_SIZE], done.srcMac, N2N_MAC_SIZE);
                    
                    HASH_FIND_PAIRS(comm->punch_pairs, pairs1_id, pairs1);
                    HASH_FIND_PAIRS(comm->punch_pairs, pairs2_id, pairs2);
                    if (pairs1 != NULL) {
                        HASH_DEL(comm->punch_pairs, pairs1);
                        free(pairs1);
                    }
                    if (pairs2 != NULL) {
                        HASH_DEL(comm->punch_pairs, pairs2);
                        free(pairs2);
                    }
                }
            }
