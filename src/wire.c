...
/* Common */
int encode_PUNCH_HELLO (uint8_t *base,
                            size_t *idx,
                            const n2n_common_t *common,
                            const n2n_PUNCH_HELLO_t *reg)
{
    int retval = 0;

    retval += encode_common(base, idx, common);
    retval += encode_mac(base, idx, reg->srcMac);
    retval += encode_mac(base, idx, reg->peerMac);
    return retval;
}

int decode_PUNCH_HELLO (n2n_PUNCH_HELLO_t *reg,
                             const n2n_common_t *cmn, /* info on how to interpret it */
                             const uint8_t *base,
                             size_t *rem,
                             size_t *idx) {
    ...
    size_t retval = 0;
    memset(reg, 0, sizeof(n2n_PUNCH_HELLO_t));
    retval += decode_mac(reg->srcMac, base, rem, idx);
    retval += decode_mac(reg->peerMac, base, rem, idx);
    return retval;
}

int encode_PUNCH_METHOD (uint8_t *base,
                            size_t *idx,
                            const n2n_common_t *common,
                            const n2n_PUNCH_METHOD_t *reg)
{
    int retval = 0;
    ...
    retval += encode_mac(base, idx, reg->srcMac);
    retval += encode_mac(base, idx, reg->peerMac);
    retval += encode_uint32(base, idx, reg->punch_method);
    return retval;
}

int decode_PUNCH_METHOD (n2n_PUNCH_METHOD_t *reg,
                             const n2n_common_t *cmn, /* info on how to interpret it */
                             const uint8_t *base,
                             size_t *rem,
                             size_t *idx) {
    ...
    size_t retval = 0;
    memset(reg, 0, sizeof(n2n_PUNCH_METHOD_t));
    retval += decode_mac(reg->srcMac, base, rem, idx);
    retval += decode_mac(reg->peerMac, base, rem, idx);
    retval += decode_uint32(&(reg->punch_method), base, rem, idx);
    return retval;
}

int encode_PUNCH_CHANGE (uint8_t *base,
                            size_t *idx,
                            const n2n_common_t *common,
                            const n2n_PUNCH_CHANGE_t *reg)
{
    int retval = 0;
...
int decode_PUNCH_CHANGE (n2n_PUNCH_CHANGE_t *reg,
                             const n2n_common_t *cmn, /* info on how to interpret it */
                             const uint8_t *base,
                             size_t *rem,
                             size_t *idx) {
    ...
    size_t retval = 0;
    memset(reg, 0, sizeof(n2n_PUNCH_CHANGE_t));
    retval += decode_mac(reg->srcMac, base, rem, idx);
    retval += decode_mac(reg->peerMac, base, rem, idx);
    return retval;
}

int encode_PUNCH_DONE (uint8_t *base,
                            size_t *idx,
                            const n2n_common_t *common,
                            const n2n_PUNCH_DONE_t *reg)
{
    int retval = 0;
...
int decode_PUNCH_DONE (n2n_PUNCH_DONE_t *reg,
                             const n2n_common_t *cmn, /* info on how to interpret it */
                             const uint8_t *base,
                             size_t *rem,
                             size_t *idx) {
    ...
    size_t retval = 0;
    memset(reg, 0, sizeof(n2n_PUNCH_DONE_t));
    retval += decode_mac(reg->srcMac, base, rem, idx);
    retval += decode_mac(reg->peerMac, base, rem, idx);
    return retval;
}

int encode_P2P_PING(uint8_t *base,
                           size_t *idx,
                           const n2n_common_t *common,
                           const n2n_P2P_PING_t *reg) {
...
int decode_P2P_PING (n2n_P2P_PING_t *reg,
                             const n2n_common_t *cmn, /* info on how to interpret it */
                             const uint8_t *base,
                             size_t *rem,
                             size_t *idx) {
    ...
    size_t retval = 0;
    memset(reg, 0, sizeof(n2n_P2P_PING_t));
    retval += decode_mac(reg->srcMac, base, rem, idx);
    return retval;
}
/* Common End */
...
int encode_N4_HELLO (uint8_t *base,
                            size_t *idx,
                            const n2n_common_t *common,
                            const n4_HELLO_t *reg)
{
    int retval = 0;
...
int decode_N4_HELLO (n4_HELLO_t *reg,
                             const n2n_common_t *cmn, /* info on how to interpret it */
                             const uint8_t *base,
                             size_t *rem,
                             size_t *idx) {
    ...
    size_t retval = 0;
    memset(reg, 0, sizeof(n4_HELLO_t));
    retval += decode_mac(reg->srcMac, base, rem, idx);
    retval += decode_mac(reg->peerMac, base, rem, idx);
...
int encode_N4_PIINFO (uint8_t *base,
                            size_t *idx,
                            const n2n_common_t *common,
                            const n4_PIINFO_t *reg) {

    int retval = 0;

    retval += encode_common(base, idx, common);
    retval += encode_mac(base, idx, reg->peerMac);
    retval += encode_uint32(base, idx, reg->peerPort);
    retval += encode_sock(base, idx, &(reg->sock));
    retval += encode_uint16(base, idx, reg->n4_type);
    retval += encode_uint16(base, idx, reg->peer_min_port);
    retval += encode_uint16(base, idx, reg->peer_max_port);
    return retval;
}

int decode_N4_PIINFO (n4_PIINFO_t *reg,
                             const n2n_common_t *cmn, /* info on how to interpret it */
                             const uint8_t *base,
                             size_t *rem,
                             size_t *idx) {
    ...
    size_t retval = 0;
    memset(reg, 0, sizeof(n4_PIINFO_t));

    retval += decode_mac(reg->peerMac, base, rem, idx);
    retval += decode_uint32(&(reg->peerPort), base, rem, idx);
    retval += decode_sock(&(reg->sock), base, rem, idx);
    retval += decode_uint16(&(reg->n4_type), base, rem, idx);
...
/* N4 End */
/* Juice */
int encode_SDP (uint8_t *base,
                            size_t *idx,
                            const n2n_common_t *common,
                            const juice_SDP_t *reg)
{
    int retval = 0;
    ...
    retval += encode_mac(base, idx, reg->srcMac);
    retval += encode_mac(base, idx, reg->peerMac);
    retval += encode_buf(base, idx, reg->sdp, N2N_MSG_BUF_SIZE);
    return retval;
}

int decode_SDP (juice_SDP_t *reg,
                             const n2n_common_t *cmn, /* info on how to interpret it */
                             const uint8_t *base,
                             size_t *rem,
                             size_t *idx) {
    ...
    size_t retval = 0;
    memset(reg, 0, sizeof(juice_SDP_t));
    retval += decode_mac(reg->srcMac, base, rem, idx);
    retval += decode_mac(reg->peerMac, base, rem, idx);
    retval += decode_buf(reg->sdp, N2N_MSG_BUF_SIZE, base, rem, idx);
    return retval;
}

int encode_CANDIDATE (uint8_t *base,
                            size_t *idx,
                            const n2n_common_t *common,
                            const juice_CANDIDATE_t *reg)
{
    int retval = 0;
    ...
    retval += encode_mac(base, idx, reg->srcMac);
    retval += encode_mac(base, idx, reg->peerMac);
    retval += encode_buf(base, idx, reg->candidate, N2N_MSG_BUF_SIZE);
    return retval;
}

int decode_CANDIDATE (juice_CANDIDATE_t *reg,
                             const n2n_common_t *cmn, /* info on how to interpret it */
                             const uint8_t *base,
                             size_t *rem,
                             size_t *idx) {
    ...
    size_t retval = 0;
    memset(reg, 0, sizeof(juice_CANDIDATE_t));
    retval += decode_mac(reg->srcMac, base, rem, idx);
    retval += decode_mac(reg->peerMac, base, rem, idx);
    retval += decode_buf(reg->candidate, N2N_MSG_BUF_SIZE, base, rem, idx);
    return retval;
}
/* Juice End */
...
int encode_REGISTER_SUPER (uint8_t *base,
    ...
    retval += encode_buf(base, idx, reg->encrypt_key, KHALA_MAX_BUF);
    retval += encode_uint32(base, idx, reg->punch_caps);
...
int decode_REGISTER_SUPER (n2n_REGISTER_SUPER_t *reg,
    ...
    retval += decode_buf(reg->encrypt_key, KHALA_MAX_BUF, base, rem, idx);
    retval += decode_uint32(&(reg->punch_caps), base, rem, idx);
